...
def init_db():
    ...
    # تحديث جدول المشرفين ليشمل الصلاحيات والاسم (سيتم إعادة إنشائه ليناسب الهيكلة الجديدة)
...
def main_menu(user_id):
    ...
    # عرض الأزرار فقط للمشرفين الذين يملكون الصلاحية
    buttons = []
...
def callback_handler(call):
    ...
    if call.data == "admin_panel":
        ...
        target_id = int(call.data.split(":")[1])
        bot.edit_message_text(f"⚙️ تحكم بصلاحيات المستخدم ({target_id}):", 
                              call.message.chat.id, call.message.message_id, 
                              reply_markup=get_permissions_keyboard(target_id))
...
def process_add_admin(message):
    try:
        ...
        bot.reply_to(message, f"✅ تم إضافة المشرف: {name}\nاذهب للوحة المشرفين -> صلاحيات المشرفين لتفعيل أزراره.")
    except Exception:
...
def process_restore(message):
    if message.document:
        ...
        bot.reply_to(message, "⚠️ خطأ: يجب إرسال ملف .db")
