...
def callback_handler(call):
    ...
    if call.data == "edit_anime":
        if has_permission(user_id, 'can_add_anime'):
            ...
            bot.answer_callback_query(call.id, "لا تملك صلاحية الإضافة/التعديل!")
        return
    ...
    if call.data == "admin_panel":
        ...
        if has_permission(user_id, 'can_add_anime'):
            ...
            bot.answer_callback_query(call.id, "لا تملك صلاحية الإضافة!")
        ...
        if has_permission(user_id, 'can_delete_anime'):
            ...
            bot.answer_callback_query(call.id, "لا تملك صلاحية الحذف!")
        ...
        if has_permission(user_id, 'can_backup'):
            try:
                ...
                bot.send_message(call.message.chat.id, "خطأ في النسخ الاحتياطي")
        else:
            bot.answer_callback_query(call.id, "لا تملك صلاحية النسخ الاحتياطي!")
        ...
        if has_permission(user_id, 'can_restore'):
            ...
            bot.answer_callback_query(call.id, "لا تملك صلاحية الاسترداد!")
        ...
        if has_permission(user_id, 'can_list'):
            ...
            bot.answer_callback_query(call.id, "لا تملك صلاحية عرض القائمة!")
...
def process_add_admin(message):
    try:
        ...
        bot.reply_to(message, "⚠️ خطأ في الصيغة. أرسل بالشكل: 12345,الاسم")
